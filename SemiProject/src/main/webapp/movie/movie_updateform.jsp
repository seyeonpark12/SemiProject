...
<body>
	<div style="margin-top: 100px; padding: 0;">
		<form action="movie/movie_updateaction.jsp" method="post"
			...
			<input type="hidden" name="before_poster"
				value="<%=dto.getMovie_poster()%>">
		...
		<table
				...
				<tr>
					...
					<a style="display: inline-block; width: 100%; height: 390px; padding: 10px; overflow: hidden; border-radious:10px;">
							<img id="showimg" src="movie_save/<%=dto.getMovie_poster()%>" class="posterimg" style="width:100%;  border-radious:10px;">
					</a>
					...
					<input type="text" name="movie_subject"
						...
						placeholder="영화제목" style="margin-bottom: 10px;" value="<%=dto.getMovie_subject()%>"> 
					
					
					
						<input
						type="date" name="movie_year" id="movie_year" required="required"
						class="form-control" value="<%=dto.getMovie_year()%>"
						...
						<option value="romance"
								<%=dto.getMovie_genre().equals("romance")?"selected":""%>>로맨스</option>
							<option value="action"
								<%=dto.getMovie_genre().equals("action")?"selected":""%>>액션</option>
							<option value="comedy"
								<%=dto.getMovie_genre().equals("comedy")?"selected":""%>>코미디</option>
							<option value="horror"
								<%=dto.getMovie_genre().equals("horror")?"selected":""%>>공포</option>
							<option value="animation"
								<%=dto.getMovie_genre().equals("animation")?"selected":""%>>애니메이션</option>
							<option value="etc"
								<%=dto.getMovie_genre().equals("etc")?"selected":""%>>기타</option>
					</select> 
					
					<input type="text" name="movie_nara" id="movie_nara"
						required="required" class="form-control" placeholder="국가"
						style="margin-bottom: 10px;" value="<%=dto.getMovie_nara()%>">
						
						<input type="text" name="movie_director" id="movie_director"
						class="form-control" required="required"
						placeholder="감독" style="margin-bottom: 10px;" value="<%=dto.getMovie_director()%>">
						
						<input type="text" name="movie_actor" id="movie_actor"
						class="form-control" required="required"
						placeholder="출연진" style="margin-bottom: 10px;" value="<%=dto.getMovie_actor()%>">
						
						<input type="text" name="movie_play" id="movie_play"
					class="form-control" required="required" 
					placeholder="유튜브 트레일러(링크)" style="margin-bottom: 10px;" value="<%=dto.getMovie_play()%>">
					</td>
				</tr>
			</table>
			...
			<div class="write" style="align: center; margin: 0 300px;">
				<textarea
					...
					name="movie_content" required="required" id="movie_comtent"><%=dto.getMovie_content()%></textarea>
			</div>



				<button type="submit" class="btn btn-default btn-ls"
					style="float: right; margin-top: 10px; margin-right: 700px;">영화수정</button>
